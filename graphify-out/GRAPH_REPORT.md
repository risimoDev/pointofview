...
## Corpus Check
- 139 files · ~69,413 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1215 nodes · 2278 edges · 80 communities (69 shown, 11 thin omitted)
- Extraction: 88% EXTRACTED · 12% INFERRED · 0% AMBIGUOUS · INFERRED: 262 edges (avg confidence: 0.53)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `675ae6ff`
...
Cohesion: 0.19
Nodes (12): CameraConfig, Settings, Detections, Frame, FileSource, Frame, RTSP pull via OpenCV with exponential-backoff reconnect.      One dead camera ne, Local file source for tests. loop=True replays the file endlessly. (+4 more)
...
Cohesion: 0.07
Nodes (27): AppUser, Bbox, cameraStatusEnum, deploymentModeEnum, eventSeverityEnum, eventTypeEnum, featureIdEnum, NewAlertRule (+19 more)
...
Cohesion: 0.04
Nodes (72): StatusDot(), AdminSite, AdminSiteSchema, AdminSitesSchema, AdminUser, AdminUserSchema, AdminUsersSchema, AlertRuleSchema (+64 more)
...
Cohesion: 0.07
Nodes (25): ndarray, ndarray, Settings, ndarray, Redis, Settings, cosine(), HistogramEmbedder (+17 more)

### Community 13 - "API Auth (JWT)"
Cohesion: 0.15
Nodes (12): 1. Текущее состояние (что работает), 2. Нерешённые проблемы (проверить в первую очередь), 3. Принятые решения (не пересматривать без причины), 4. План улучшений (приоритетный), 5. Полезное для новой сессии, ViziAI — план развития и контекст сессии (обновлено 2026-07-12), Блок A — быстрые победы (по дню, делать первыми), Блок B — вау + продажи (+4 more)

### Community 14 - "Alerts Worker"
Cohesion: 0.14
Nodes (24): AlertRule, Notification, AlertJob, buildText(), Channel, Channels, DigestEntry, escapeHtml() (+16 more)

### Community 15 - "Zone Editor (Canvas)"
Cohesion: 0.14
Nodes (19): CameraRow(), SOURCES, STATUSES, FEATURE_META, FeatureMeta, FieldDef, OBJECT_TYPES, getSites() (+11 more)

### Community 16 - "Events Page (Web)"
Cohesion: 0.11
Nodes (18): ClipCell(), severityVariant, ClipState, ClipStatus, useClipRequest(), getClipUrl(), getEventSnapshotUrl(), requestClip() (+10 more)
...
Cohesion: 0.14
Nodes (20): db, pool, AuditLog, Zone, AlertRuleBody, RoleEnum, eventsRoutes(), internalRoutes() (+12 more)
...
Cohesion: 0.14
Nodes (18): ArchiveSegment, DemoRequestBody, OBJECT_LABELS, publicRoutes(), checkCameras(), publish(), deleteSegments(), freeGb() (+10 more)

### Community 22 - "Cameras Routes & go2rtc Sync"
Cohesion: 0.11
Nodes (16): getWsTicket(), ApiEvent, ApiEventSchema, BboxSchema, CameraSchema, CamerasSchema, CameraStatus, EventsPage (+8 more)
...
Cohesion: 0.16
Nodes (15): buildChannels(), EMPTY_FORM, EVENT_TYPES, eventTypeLabel(), formToInput(), NON_ALERTABLE, RuleFormState, RuleRow() (+7 more)
...
Cohesion: 0.10
Nodes (21): Site, TenantFeature, go2rtcSource(), reconcileGo2rtc(), registerGo2rtc(), VIDEO_EXT, BboxSchema, CameraIdParams (+13 more)
...
Cohesion: 0.15
Nodes (6): faceEnrollKey(), FaceStaffJson, GalleryJson, queueFaceEnrollFromCrop(), StaffBody, StaffJson
...
Cohesion: 0.19
Nodes (13): Camera, Event, analyticsRoutes(), OverviewQuery, SummaryRow, ClipJob, SummaryQuery, settingBool() (+5 more)

### Community 67 - "Community 67"
Cohesion: 0.11
Nodes (17): FastifyInstance, FastifyJWT, FastifyRequest, JwtPayload, adminRoutes(), camerasRoutes(), startGo2rtcReconciler(), featuresRoutes() (+9 more)
...
Cohesion: 0.18
Nodes (9): getSystemInfo(), saveServerSettings(), ServerSetting, SystemInfo, AdminSettingsPage(), fmtBytes(), fmtUptime(), GROUP_LABELS (+1 more)
...
Cohesion: 0.23
Nodes (6): AnalyzerWorker, _iso(), main(), Re-read reid config/staff gallery + persist dirty embeddings/crops., Re-read enabled features so admin toggles apply without a restart.         Reuse, Single GPU process. Runs all tenant cameras concurrently via asyncio;     YOLO i

### Community 78 - "Community 78"
Cohesion: 0.25
Nodes (7): 0. Как система «учится» (важно понимать), 1. Сделано 2026-07-13, 2. Точность распознавания — следующие шаги, 3. Анти-шум алертов — следующие шаги, 4. Новый ИИ-функционал (по ценности для владельца), 5. Правила экспериментов, План улучшений ИИ-платформы

### Community 79 - "Community 79"
Cohesion: 0.29
Nodes (4): CATALOG, Feature, getFeatures(), setFeature()

## Knowledge Gaps
- **405 isolated node(s):** `VideoCapture`, `deploymentModeEnum`, `sourceTypeEnum`, `cameraStatusEnum`, `zoneKindEnum` (+400 more)
...
- **Why does `Settings` connect `Zone Engine` to `Analyzer Config & Settings`, `API Events Routes & Queues`, `Community 73`, `Video Grid (WebRTC)`, `Community 77`?**
  _High betweenness centrality (0.025) - this node is a cross-community bridge._
- **Why does `IdentityManager` connect `Video Grid (WebRTC)` to `Zone Engine`, `Community 77`?**
  _High betweenness centrality (0.008) - this node is a cross-community bridge._
- **Why does `Button` connect `Zone Editor (Canvas)` to `Web API Client & Clips`, `Community 74`, `Community 79`, `Events Page (Web)`, `Community 55`, `WebSocket Hub`, `Web Auth Middleware`?**
  _High betweenness centrality (0.006) - this node is a cross-community bridge._
- **Are the 64 inferred relationships involving `Settings` (e.g. with `CameraConfig` and `Settings`) actually correct?**
